...
RSpec.describe KitFacade, type: :facade do
  ...
  before do
    @record = Kit.create(name: "Andromeda Strain",
  ...
  it 'gets the names of all existing kits' do
    data = KitSerializer.all_kits
    kits = data["kits"]
    expect(kits.sort).to eq (["Andromeda Strain", "Apollo 11", "Magnetosphere"])
  end

  it 'builds a hash of all the elements in a kit' do
    samples = KitFacade.get_samples(@record)
    kit = KitObject.new(@record, samples)

    elements = KitSerializer.elements_hash(kit.elements)
  ...
  it 'builds the response for a kit' do
    samples = KitFacade.get_samples(@record)
    kit_object = KitObject.new(@record, samples)

    output = KitSerializer.kit_show(kit_object)
